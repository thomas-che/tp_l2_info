...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        boolean triIsocele= false;
        boolean triRectangle= false;

        if (a!=0 && b!=0 && c!=0) {

            triEquilateral = ((a == b && b == c) ? true : false);
            if (triEquilateral) {
                System.out.println("triagle est equilateral");
            }

            triIsocele = ((a == b || a == c || c == b) ? true : false);
            if (triIsocele) {
                System.out.println("triangle est isocele");
            }

            triRectangle = (((c ^ 2) == ((b ^ 2) + (a ^ 2))) || ((b ^ 2) == ((c ^ 2) + (a ^ 2))) || ((a ^ 2) == ((b ^ 2) + (c ^ 2))) ? true : false);
            if (triRectangle) {
                System.out.println("triangle est rectangle");
            }
        }
        else {
            System.out.println("ne peux pas etre un triangle: une longeur =0");
        }
    }
}
